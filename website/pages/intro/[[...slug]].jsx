...
export async function getStaticProps({ params }) {
  const slug = ['intro', ...(params.slug || [])].join('/')
  const url = `/${slug}`
  ...
  const sidenavData = await readAllFrontMatter(`${process.cwd()}/content/intro`)
...
export async function getStaticPaths() {
  return {
    paths: await getStaticMdxPaths(`${process.cwd()}/content/intro`),
    fallback: false,
  }
}
